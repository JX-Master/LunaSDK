Elements and layers are the structural data model of GUI. They describe what exists in the submitted frame and where each subtree belongs in Z order.
...
## Concepts
### Stable ID
`GUI::id_t` is used for elements, layers and state IDs. Stable IDs should remain consistent across frames for the same logical item.

Use `IContext::make_id` or `GUI::make_scoped_id` to build IDs from local numbers or strings. Use data scopes to isolate repeated structures.

```cpp
context->push_data_scope(context->make_id("asset-list"));
GUI::id_t item_id = context->make_id(asset_guid_string);
...
```cpp
GUI::id_t popup_layer_id = context->make_id("popup-layer");
context->push_layer(popup_layer_id, popup_screen_pos);
GUI::ElementHandle popup_root = context->begin_element(context->make_id("popup-root"));
...
```cpp
GUI::ElementHandle panel = context->begin_element(context->make_id("panel"));
context->set_layout_config(panel, panel_layout);

GUI::ElementHandle label = context->begin_element(context->make_id("title"));
...
See [[GUI Input and Interaction]] for routing behavior.
...
GUI::ElementHandle handle = context->find_element_handle(id);
```
...
if(open_popup)
{
    ...
    GUI::ElementHandle root = context->begin_element(context->make_id("file-popup-root"));
...
```cpp
GUI::ElementHandle list = context->begin_element(context->make_id("tracks"));
context->push_data_scope(list.id);

for(usize i = first_visible; i < last_visible; ++i)
{
    context->push_data_scope(GUI::make_scoped_id(list.id, track_ids[i]));
    GUI::ElementHandle row = context->begin_element(context->make_id("row"));

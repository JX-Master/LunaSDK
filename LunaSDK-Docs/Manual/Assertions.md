...
All those four assertion macros take effect only when the target is built with debug-level checks enabled through LunaBuild configuration defines such as `LUNA_DEBUG_LEVEL`. If you want these macros to work in profile and release builds, use macros with `_always` suffix, like `luassert_always`, `lupanic_always`, `luassert_msg_always` and `lupanic_msg_always`.
...
For such case, `lucheck` and `lucheck_msg` can be used to check external programming mistakes. `lucheck` and `lucheck_msg` behave the same as `luassert` and `luassert_msg`. However, these two macros are not controlled by `LUNA_DEBUG`, but by `LUNA_ENABLE_API_VALIDATION`. LunaBuild enables this macro automatically for Debug builds, and it can be enabled explicitly for other modes with `--api-validation` or the compatibility alias `--contract-assertion`. The module developer can place `lucheck` and `lucheck_msg` at the beginning of the function implementation to validate function arguments and calling time, and interrupts the program if the function is improperly called. With these macros, the developer of the module may ship two versions of libraries to the user, one for developing with the module, and another for releasing with the final product. The development version of the module can be compiled on release mode with `--api-validation`, so all `luassert` assertions get removed, but `lucheck` assertions are retained for the user, while the release version of the module will remove both `luassert` and `lucheck` for maximum performance.

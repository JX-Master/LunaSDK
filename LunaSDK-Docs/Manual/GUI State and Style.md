GUI provides state storage and named styles shared across GUI packages.
...
GUI does not define widget-specific state or widget-specific style keys. Higher-level packages define those objects and keys.
...
```cpp
GUI::id_t state_id = GUI::make_state_id<StateType>(owner_id);
```

This combines the owner ID with the boxed state type GUID.

### State lifetime
`GUI::StateLifetime` controls cleanup:
...
### Style
`GUI::Style` is a named set of entries and an optional parent style. Entries can:
...
The `EditorGUI` package registers and initializes `gui.editor.default` when
`EditorGUI::register_style_schemas` is called. GUI elements without an explicit style binding resolve their values from
this package-owned style. It provides light and dark palettes, a configurable accent and separate pointer and touch
density metrics.

Use `EditorGUI::set_default_style` to switch the shared defaults:

```cpp
EditorGUI::DefaultStyleDesc desc;
desc.color_theme = EditorGUI::ColorTheme::dark;
desc.input_mode = EditorGUI::InputMode::pointer;
desc.accent = Float4U(0.89f, 0.31f, 0.35f, 1.0f);
EditorGUI::set_default_style(context, desc);
```
...
### Style schema
`StyleEntrySchema` describes a style entry consumed by a package. GUI stores schemas for tools and inspectors. It does not interpret the entry as behavior.
...
GUI::id_t state_id = GUI::make_state_id<MyControlState>(element.id);
Ref<MyControlState> state = new_object<MyControlState>();
state->animation = 0.5f;
luexp(context->set_state(state_id, state.object(), GUI::StateLifetime::next_frame));
...
context->set_style_value(Name("editor.dark"),
    ...
    GUI::style_f32x4(Float4U(0.90f, 0.93f, 0.96f, 1.0f)));
```
...
GUI::ElementHandle button = context->begin_element(button_id);
context->end_element();
context->pop_style();
```
...
```cpp
GUI::StyleValue fallback = GUI::style_f32x4(Float4U(1.0f));
GUI::StyleValue value = context->get_style_value(
...
schema.type = GUI::StyleValueType::f32x4;
schema.default_value = GUI::style_f32x4(Float4U(0.12f, 0.18f, 0.26f, 1.0f));
...
```cpp
for(const GUI::Element& element : context->get_elements())
{
    if(element.id == selected_element_id)
    {
        for(const GUI::StyleEntrySchema& schema : context->get_style_entry_schemas())
        {
            GUI::StyleValue value = context->get_style_value(

GUI layout algorithms operate on typeless element data. They read dense `LayoutConfig` input and optional sparse `LayoutCallbackConfig` records, measure child size requirements when needed, then write `LayoutResult` back to the context.
...
GUI layout helpers are intentionally primitive. A high-level package may combine them with widget-specific measurement, scrollbars, table headers, virtualization or editor metadata.

## Concepts
### Layout config
`GUI::LayoutConfig` is attached directly to each element. It contains only frequently accessed input:
...
Optional callbacks use `GUI::LayoutCallbackConfig`, stored in a per-context sparse array. It contains:
...
GUI invokes callback pointers directly. `algorithm` is not a registration or dispatch key. An element without callbacks stores no sparse record.
...
### Measure callback
`LayoutCallbackConfig::measure_callback` computes content-box size for one element. It returns a `GUI::MeasureResult`:
...
GUI adds padding, applies `SizeValue::min` and `SizeValue::max`, and resolves the final element-box measurement. Margin belongs to the parent layout algorithm and is not included in `MeasureResult`. A parent layout algorithm reserves the child's margin before calling `IContext::measure_element`; the measure callback therefore receives the remaining content-box size after GUI removes padding.

This keeps drawing and measurement separate. Text may live only in `DrawCommand` records; GUI will not scan draw commands to infer text size. A higher-level package that wants text hug sizing should install a text measure callback and pass the text, font and font size through its own callback userdata.
...
`clip_rect` participates in hit testing and is also applied automatically while GUI compiles each element-owned draw command. Explicit `push_clip` and `pop_clip` commands add a stricter nested clip; they do not replace layout clipping.
...
config.width.kind = GUI::SizeKind::percent;
config.width.value = 1.0f;
config.height.kind = GUI::SizeKind::fixed;
...
GUI::MeasureResult measure_text(
...
label_layout.width.kind = GUI::SizeKind::fit;
label_layout.height.kind = GUI::SizeKind::fit;
label_layout.padding = Float4U(8.0f, 4.0f, 8.0f, 4.0f);
context->set_layout_config(label, label_layout);

GUI::LayoutCallbackConfig label_callbacks;
...
row_layout.width.kind = GUI::SizeKind::percent;
row_layout.width.value = 1.0f;
row_layout.height.kind = GUI::SizeKind::fit;
context->set_layout_config(row, row_layout);

GUI::LayoutCallbackConfig row_callbacks;
row_callbacks.algorithm = Name("gui.core.flex");
row_callbacks.measure_callback = GUI::measure_flex;
row_callbacks.callback = GUI::layout_flex;
...
desc.mode = GUI::GridLayoutMode::fixed_cell_size;
desc.cell_size = Float2U(96.0f, 80.0f);
desc.gap = Float2U(8.0f, 8.0f);

GUI::LayoutCallbackConfig grid_callbacks;
grid_callbacks.algorithm = Name("gui.core.grid");
grid_callbacks.callback = GUI::layout_grid;
...
```cpp
desc.mode = GUI::GridLayoutMode::fixed_column_count;
desc.column_count = 4;
...
desc.items = Span<const GUI::CanvasLayoutItem>(&item, 1);

GUI::LayoutCallbackConfig canvas_callbacks;
canvas_callbacks.algorithm = Name("gui.core.canvas");
canvas_callbacks.callback = GUI::layout_canvas;
...
Higher-level packages should implement scroll state, scrollbars, clamping and data slicing on top of this primitive. GUI receives only the submitted element subset and does not track item counts, indexes or virtualization state.

When scroll clamping or a scrollbar needs the complete content extent, submit one content element whose layout size represents the complete data set, then slice only that element's descendants. Place the submitted descendants at their absolute content offsets. This keeps the complete scroll range visible to ordinary layout code without introducing item or virtualization semantics into GUI.
...
GUI::TableLayoutDesc desc;
desc.columns = Span<const GUI::TableTrackDesc>(columns, 3);
desc.rows = Span<const GUI::TableTrackDesc>(rows, row_count);
desc.cells = Span<const GUI::TableLayoutCell>(cells, cell_count);
desc.gap = Float2U(4.0f, 2.0f);

GUI::LayoutCallbackConfig table_callbacks;
table_callbacks.algorithm = Name("gui.core.table");
table_callbacks.callback = GUI::layout_table;
...
GUI::LayoutConfig panel_layout;
panel_layout.width.kind = GUI::SizeKind::fixed;
panel_layout.width.value = 280.0f;
panel_layout.height.kind = GUI::SizeKind::percent;
panel_layout.height.value = 1.0f;

GUI::ElementHandle panel = context->begin_element(context->make_id("inspector"));
context->set_layout_config(panel, panel_layout);

GUI::LayoutCallbackConfig panel_callbacks;
panel_callbacks.algorithm = Name("gui.core.flex");
panel_callbacks.measure_callback = GUI::measure_flex;
panel_callbacks.callback = GUI::layout_flex;
...
GUI table layout can place only the rows that were submitted. A higher-level package can compute visible row range, submit only those children, then provide row tracks and cell attachments for that range.

...
`copy_file(from, to, destination)` and `move_file(from, to, destination, flags)` may handle another instance directly; an omitted or null destination selects the current instance. VFS tries these methods before streaming. Returning `E_NOT_SUPPORTED` must leave storage unchanged. Unless `FileMoveFlag::no_copy` forbids fallback, VFS can stream a file to an absent destination. The generic fallback never replaces an existing destination. Other errors propagate directly. Concrete implementations decide which destination objects they support; the router keeps no type registry. `get_native_path` returns `E_NOT_SUPPORTED` for entries without independent native paths.

`VFS::move_file(from, to, flags)` rejects existing destinations by default. `FileMoveFlag::allow_overwrite` permits compatible native moves and same-Pak moves to replace an existing file; it does not enable replacing a directory. Combine it with `no_copy` when publishing a completed temporary file so an unsupported move cannot fall back to copying into the destination. Native and Pak implementations validate replacement before mutation; affected open Pak files return `E_BUSY`.

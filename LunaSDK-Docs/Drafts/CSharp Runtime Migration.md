...
29. ShaderCompiler now has its own module proxy. The first slice exposes `ShaderCompilerModule.Init`, `ShaderCompileParameters`, `ShaderCompileResult`, and the compile target/type/optimization/matrix/Metal platform enums. The result is copied into managed `byte[]` immediately and the native compile buffers are freed before returning, so C# tests can feed shader bytecode into `ShaderData` without depending on C++ generated shader headers.
30. `RHICSharpTest` now contains the managed RHITest2 triangle path: it compiles HLSL vertex/pixel shader source at runtime through `Luna.ShaderCompiler`, creates a real graphics pipeline layout and PSO, writes the triangle vertex buffer from managed bytes, binds pipeline state/layout/vertex buffers, sets viewport/scissor, and records a draw call in the existing render pass.
31. Image decoding has a first managed proxy (`Luna.Image`) that can initialize the native Image module and decode image file bytes or paths into managed `byte[]` pixel data. `RHICSharpTest` now uses it together with ShaderCompiler, RHIUtility resource uploads/readback, descriptor sets, texture sampling, index buffers, uniform buffers, and a depth attachment to cover the core RHITest3 texture path and RHITest4 textured-box path. The texture cases read uploaded textures back and compare them with decoded image bytes, and the shared setup validates blit/mipmap context creation, descriptor arrays and read-write descriptors, naming, and `IDeviceChild.Device`. Shader-to-RHI convenience helpers and fuller standalone managed RHITest sample parity remain follow-up work.

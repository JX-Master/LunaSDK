...
48. VariantUtils now has a first managed bridge (`Luna.VariantUtils`) for the public `VariantUtils.hpp`, `JSON.hpp`, `XML.hpp`, and `Diff.hpp` surface, together with a minimal managed `Variant` proxy implemented as an opaque native heap object rather than by reviving the old C++ reflection/serialization system. The current API exposes module init, `VariantType` / `VariantNumberType`, variant creation and cloning, equality, basic object/array mutation and traversal helpers, string/number/Boolean/blob accessors, JSON and XML read/write from strings or `IStream`, XML element helpers, and diff/patch/revert plus diff-prefix composition. `VariantUtilsCSharpTest` validates JSON roundtrips, blob and negative-number handling, XML roundtrips and mixed-content parsing, diff/patch/revert, and basic XML element mutation on a desktop run.

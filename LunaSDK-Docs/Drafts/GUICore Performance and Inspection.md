GUI Core exposes operational performance counters in every build and direct read-only access to the current frame data for inspection tools.
...
GUI Core does not provide issue/pass logs, input recording, input replay, frame timelines or a cross-process transport format.
...
for(u32 i = 0; i < elements.size(); ++i)
{
    ...
    GUICore::LayoutCallbackConfig layout_callbacks = context->get_layout_callback_config(handle);
...
for(GUICore::id_t scope : context->get_data_scope_stack())
{
    // Inspect the active scoped-ID path.
}

const HashMap<Name, GUICore::Style>& styles = context->get_styles();
for(auto iter = styles.begin(); iter != styles.end(); ++iter)
{
    const Name& style_name = iter->first;
    const GUICore::Style& style = iter->second;
...
```cpp
for(const GUICore::DrawCommand& command : context->get_draw_commands())
{
    if(command.element != GUICore::INVALID_ELEMENT)
    {
        const GUICore::Element* owner = context->get_element(command.element);

...
GUI layout, drawing and input use a two-dimensional logical coordinate system. This is appropriate for desktop
...
antialiasing from perspective-correct interpolated shape coordinates, while the initial GUI SDF path mapped
instance rectangles directly from logical screen coordinates to clip space. A complete core rendering contract must
therefore support window and world-space surfaces without coupling GUI to a game camera, physics world or scene
...
2. `EditorGUI::RenderProxy`
   - High-level immediate APIs attach package-owned GUI draw callbacks or submit static commands directly.
   - Rendering policy is part of the immediate API package implementation, not a fully replaceable per-widget runtime callback.

3. `EditorGUI::IContext`
   - Context ownership, frame state, input, layout, state, style, and rendering are `GUI` responsibilities.
   - High-level GUI APIs take a `GUI` context pointer or wrapper handle and operate on the core element tree.

### Two-layer GUI architecture
`GUI` is internally divided into a logical layer and a rendering layer. These are architectural subsystems of the same module rather than two competing GUI contexts or element trees.
...
compressed or depth formats fail explicitly. GUI filters the numeric values supplied by the texture format and
...
submit selected ranges into GUI-owned passes. GUI combines those usages with SDF and backdrop resources and
records the actual barriers.

### SDF shape and paint programs
GUI owns a signed-distance-field backend for analytic GUI geometry and effects. VG remains the backend for text,
...
SDF and integrated VG pipelines derive compatible color and depth formats from the supplied attachments. GUI
...
GUI does not accept camera rays or query a physics world. The host selects a surface, intersects the pointer ray
...
`GUI` owns the following systems:
...
9. **Inspection and performance instrumentation**
   ...
   - Does not own issue/pass trace logs, input recording, input replay, frame timelines, or a cross-process debug transport. Such tools may be implemented outside GUI when a concrete use case requires them.

### Data and algorithm separation
`GUI` follows a data-oriented model. The element tree is a data container. Core features are algorithms that operate on this data.
...
`LayoutCallbackConfig::algorithm` is semantic metadata for diagnostics and higher-level capability recognition. GUI dispatches through callback pointers and never uses the name as a callback registry key.

### Per-element navigation policy
GUI consumes semantic navigation input rather than hard-wiring platform keys. Host adapters translate keyboard,
gamepad or other input into directional navigation, sequential forward/backward navigation, confirm and back actions.

GUI does not store a context-level navigation graph. Explicit relationships are usually view- and
...
The existing `EditorGUI` module becomes one high-level immediate API package built on top of `GUI`.
...
[[ADR-0015 Introduce GameGUI and GameGUIEditor]].
...
presentation. GUI owns attachment transitions and every render/compute pass recorded by its renderer.
...
complete dependency and service boundary is proposed by [[ADR-0015 Introduce GameGUI and GameGUIEditor]].
...
Migrate the simplest APIs in the default editor-style `EditorGUI` package:
...
These APIs should submit GUI element data, interactable data, and layout data, then attach package-owned GUI draw callbacks or static commands. They should not create `EditorGUI::Node` objects or assign `EditorGUI::RenderProxy` objects.
...
12. **Host-independent world input**: ray-to-surface mapping reuses the existing logical input router without adding camera, scene or physics dependencies to GUI.
...
15. **View-owned explicit navigation**: applications can block or override individual navigation actions without making GUI own a business-specific graph.
...
9. **Surface-transform integration risk**: custom transforms must follow GUI's top-left coordinate convention
...
13. Keep scene selection, ray ordering, camera composition and IME screen projection in host adapters rather than GUI.
...
### Require VG to remain the exclusive GUI rasterization backend
* Status: rejected.
* Pros:
...
* Pros:
    ...
    2. Does not require GUI to begin or end the application's target pass.
* Cons:
...
* Pros:
    ...
    2. Package authors can introduce effects without changing GUI command types.
* Cons:
    1. Arbitrary callbacks are not inspectable or serializable command data.
    2. Resource usage, barriers, clipping, and painter order become difficult for GUI to validate and schedule.
...
### Continue evolving the current EditorGUI module
* Status: rejected.
* Pros:
...
* Cons:
    ...
    4. This conflicts with [[ADR-0015 Introduce GameGUI and GameGUIEditor]], which gives GameGUI its own editable
       semantic node types.
...
### Keep EditorGUI::IContext as the high-level context
* Status: rejected.
* Pros:
...
* Status: rejected.
* Pros:
    1. Makes explicit edges easy for GUI tools to visualize and validate.
* Cons:
    1. Adds graph ownership, lifetime, validation and cross-frame cleanup to the core.
    2. Encourages business-specific relationships to leak into GUI.
...
* Status: rejected.
* Pros:
    1. Lets GUI perform surface intersection internally.
* Cons:
...
High-level package decisions remain separate; [[ADR-0015 Introduce GameGUI and GameGUIEditor]] proposes GameGUI and
GameGUIEditor without changing GUI's primitive-only responsibility.

## Version history
* **2026/8/29** Updated the GameGUI cross-reference after the GameGUI decision was renumbered to ADR-0015.
...
* **2026/7/30** Replaced the application-owned `prepare`/`render` split with one GUI-owned rendering entry point.
  ...
  fast path.
* **2026/7/22** Added the default EditorGUI package's curated Phosphor-derived vector icon pack, offline SVG-to-VG generation contract and ordinary-child composition rule; widget containers remain icon-agnostic.
* **2026/7/22** Consolidated the previously separate per-element navigation and SDF shape/paint decisions into ADR-0004, and established this ADR as the single pre-release architecture record for GUI.
* **2026/7/22** Integrated the world-space GUI surface decision into this ADR. GUI now defines one top-left logical surface per context, a shared SDF/VG surface-to-clip transform, perspective derivative antialiasing, optional scene depth and host-side ray-to-surface input mapping.
...
* **2026/7/17** Split GUI internally into a logical layer and a rendering layer. The logical layer generates ordered primitive draw commands, while the rendering layer owns command compilation and backend interleaving and may combine VG rasterization with explicit GUI-owned RHI pipelines for shadows, offscreen composition, and future multi-pass effects.
* **2026/7/13** Made human-readable debug names and high-level debug views resident in all builds, and removed the dedicated GUI debug compilation option. The remaining memory and update cost is small compared with the API, ABI, build, and tooling complexity caused by conditional availability.
...
* **2026/6/17** Revised the decision to remove `EditorGUI::Node`, `EditorGUI::RenderProxy`, and `EditorGUI::IContext` from the long-term architecture, define the element tree as typeless data, and narrow the default `EditorGUI` module into one editor-style immediate API package.
* **2026/6/16** Proposed.

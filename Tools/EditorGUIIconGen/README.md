# Editor GUI Icon Generator

`Generate.py` converts a curated set of Phosphor SVG assets into the scalar VG command stream embedded by the
`Luna.EditorGUI` module. The generated application binary does not contain an SVG parser or one C++ function per icon.
...
listed in `Modules/Luna/EditorGUI/Res/PhosphorIcons.json`. The upstream MIT license is copied to
`Modules/Luna/EditorGUI/Res/PhosphorIcons.LICENSE`.

Generate the checked-in files from a local checkout of the upstream repository:

```sh
python3 Tools/EditorGUIIconGen/Generate.py \
    --source /path/to/phosphor-icons-core \
    --manifest Modules/Luna/EditorGUI/Res/PhosphorIcons.json \
    --names-output Modules/Luna/EditorGUI/IconNames.inl \
    --data-output Modules/Luna/EditorGUI/Res/PhosphorCore.bin \

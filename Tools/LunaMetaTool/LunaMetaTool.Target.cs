...
public sealed class LunaMetaToolTargetRules : TargetRules
{
    public LunaMetaToolTargetRules()
        : base(
            name: "LunaMetaTool",
            targetDirectory: "Tools/LunaMetaTool",
            rulesPath: "Tools/LunaMetaTool/LunaMetaTool.Target.cs")
    {
    ...
    protected override void Configure(BuildWorkspace workspace, BuildOptions options)
    {
        if(options.Platform == BuildPlatform.MacOS)
        {
            ...
            {
                throw new DirectoryNotFoundException($"LunaMetaTool requires LLVM SDK: {llvmSdk}");
            }

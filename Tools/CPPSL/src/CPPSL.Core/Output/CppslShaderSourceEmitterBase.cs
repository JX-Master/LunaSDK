...
{
    private Dictionary<string, string> _functionNamesByDeclId = new(StringComparer.Ordinal);
    private Dictionary<string, string> _methodNamesByDeclId = new(StringComparer.Ordinal);
    private Dictionary<string, string> _structNamesBySourceType = new(StringComparer.Ordinal);

    protected void BeginShaderModelEmission(CppslShaderModel shaderModel)
    {
        _functionNamesByDeclId = shaderModel.Functions
            .Where(static function => !string.IsNullOrWhiteSpace(function.DeclId))
            .ToDictionary(
                static function => function.DeclId!,
                static function => function.EmittedName,
                StringComparer.Ordinal);
        _methodNamesByDeclId = shaderModel.Structs
            ...
            .ToDictionary(
                ...
                static method => method.EmittedName,
                StringComparer.Ordinal);
        _structNamesBySourceType = BuildStructTypeNameMap(shaderModel);
    }

    protected void EndShaderModelEmission()
    {
        _functionNamesByDeclId = new Dictionary<string, string>(StringComparer.Ordinal);
        _methodNamesByDeclId = new Dictionary<string, string>(StringComparer.Ordinal);
        _structNamesBySourceType = new Dictionary<string, string>(StringComparer.Ordinal);
    }

    private static Dictionary<string, string> BuildStructTypeNameMap(CppslShaderModel shaderModel)
    {
        var result = new Dictionary<string, string>(StringComparer.Ordinal);
        foreach (var structure in shaderModel.Structs)
        {
            if (!structure.IsTemplateInstantiation)
            {
                AddStructTypeName(result, structure.Name, structure.EmittedName);
                AddStructTypeName(result, structure.DisplayName, structure.EmittedName);
                continue;
            }

            AddStructTypeName(result, structure.DisplayName, structure.EmittedName);
            var argumentNames = structure.TemplateArguments
                .Select(static argument => argument.TypeInfo?.Spelling ?? argument.Value ?? argument.Spelling)
                .Where(static name => !string.IsNullOrWhiteSpace(name))
                .Select(static name => NormalizeShaderTypeName(name!))
                .ToArray();
            if (argumentNames.Length != 0)
            {
                AddStructTypeName(result, $"{structure.Name}<{string.Join(", ", argumentNames)}>", structure.EmittedName);
                AddStructTypeName(result, $"{structure.Name}<{string.Join(",", argumentNames)}>", structure.EmittedName);
            }
        }
        return result;
    ...
    private static void AddStructTypeName(Dictionary<string, string> map, string? sourceType, string emittedName)
    {
        ...
        var normalized = NormalizeShaderTypeName(sourceType);
        if (normalized.Length != 0)
        {
            map[normalized] = emittedName;
        }
    }
    ...
    protected string MapShaderTypeName(string type)
    {
        var normalized = NormalizeShaderTypeName(type);
        return _structNamesBySourceType.TryGetValue(normalized, out var emittedName)
            ? emittedName
            : normalized;
    }
    ...
    protected virtual string LowerCallExpression(CppslShaderModelNode node)
    {
        ...
        {
            memberName = MapMethodName(node.DirectCalleeDeclId, memberName);
            return LowerMemberCall(receiver, memberName, memberArguments);
        }

        var sourceName = node.DisplayName ?? node.Spelling;
        var name = !string.IsNullOrWhiteSpace(node.DirectCalleeDeclId) &&
            _functionNamesByDeclId.TryGetValue(node.DirectCalleeDeclId, out var mappedName)
                ? mappedName
                : sourceName;
        var arguments = node.Children
            .Skip(IsCalleeReference(node.Children.FirstOrDefault(), sourceName) ? 1 : 0)
    ...
    protected string MapMethodName(string? directCalleeDeclId, string fallback)
    {
        return !string.IsNullOrWhiteSpace(directCalleeDeclId) &&
            _methodNamesByDeclId.TryGetValue(directCalleeDeclId, out var mappedName)
                ? mappedName
                : fallback;
    }

...
if (-not (Test-Path $NativeExtractor -PathType Leaf)) {
    Write-Error "CPPSL native extractor does not exist: $NativeExtractor`nBuild it first, for example: dotnet run --project LunaBuild.csproj -- build --target cppsl-native-extractor --platform Windows --arch x64"
}

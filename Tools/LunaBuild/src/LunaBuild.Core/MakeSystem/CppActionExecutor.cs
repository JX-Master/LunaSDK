...
public sealed class CppActionExecutor : IMakeActionExecutor
{
    ...
    private async Task CompileAppleAsync(MakeActionContext context, CancellationToken cancellationToken)
    {
        ...
        var toolchain = AppleToolchain(context.Graph.Options);
        var args = CppCommandLineBuilder.BuildAppleCompileArguments(context.Workspace, context.Graph.Options, payload, toolchain.SdkPath);
    ...
    private List<string> AppleLinkArgs(MakeActionContext context, ActionPayload payload, string output, AppleClangToolchain toolchain)
    {
        ...
        if(context.Graph.Options.Platform == BuildPlatform.IOS)
        {
        ...
        if(CppCommandLineBuilder.AppleDeploymentTargetArgument(context.Graph.Options) is { } deploymentTargetArgument)
        {

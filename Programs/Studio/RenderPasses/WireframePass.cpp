...
namespace Luna
{
    ...
    RV WireframePass::execute(RG::IRenderPassContext* ctx)
    {
        ...
        lutry
        {
            ...
            for (usize i = 0; i < mesh_render_params.size(); ++i)
            {
                ...
                // Draw pieces.
                auto mesh = get_asset_or_async_load_if_not_ready<Mesh>(mesh_render_params[i].model->mesh, Name());
    ...
    RV compile_wireframe_pass(object_t userdata, RG::IRenderGraphCompiler* compiler)
    {
        lutry
        {
            ...
            if(scene_texture == RG::INVALID_RESOURCE) return set_error(E_BAD_ARGUMENTS, "WireframePass: Output \"scene_texture\" is not specified.");

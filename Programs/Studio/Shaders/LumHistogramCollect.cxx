...
[[cppsl::rwstructured_buffer, cppsl::desc_set(0), cppsl::binding(1)]]
uint* g_histogram;
...
{
    uint count_for_this_bin = group_index > 0u ? g_histogram[group_index] : 0u;
    ...
    if (group_index == 0u)
    {
        float weighted_log_average = (float(histogram_shared[0]) / max(cb.num_pixels - float(count_for_this_bin), 1.0f)) - 1.0f;
        float min_log_lum = log2(cb.min_brightness);
        float log_lum_range = log2(cb.max_brightness) - min_log_lum;
        float weighted_avg_lum = exp2(((weighted_log_average / 254.0f) * log_lum_range) + min_log_lum);

        float lum_last_frame = g_target.Load(uint2{0u, 0u});
        float adapted_lum = lum_last_frame == 0.0f ? weighted_avg_lum : (lum_last_frame + (weighted_avg_lum - lum_last_frame) * cb.time_coeff);
        g_target.Store(uint2{0u, 0u}, adapted_lum);
    }
}

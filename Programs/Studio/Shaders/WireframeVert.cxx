...
PS_INPUT vs_main(MeshVertex input)
{
    ...
    world_position4 = mul(g_MeshBuffer[0].model_to_world, float4{input.position.x, input.position.y, input.position.z, 1.0f});
    output.world_position = float3{world_position4.x, world_position4.y, world_position4.z};
    output.position = mul(g_cb.world_to_proj, float4{output.world_position.x, output.world_position.y, output.world_position.z, 1.0f});

    float4 normal4;
    normal4 = mul(float4{input.normal.x, input.normal.y, input.normal.z, 0.0f}, g_MeshBuffer[0].world_to_model);
    output.normal = float3{normal4.x, normal4.y, normal4.z};

    float4 tangent4;
    tangent4 = mul(float4{input.tangent.x, input.tangent.y, input.tangent.z, 0.0f}, g_MeshBuffer[0].world_to_model);

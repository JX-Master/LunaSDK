...
{
    uint count_for_this_bin = (group_index > 0) ? g_histogram[group_index] : 0;
    ...
    if (group_index == 0) 
    {
        ...
        float adapted_lum = (lum_last_frame == 0.0f) ? weighted_avg_lum : (lum_last_frame + (weighted_avg_lum - lum_last_frame) * time_coeff);

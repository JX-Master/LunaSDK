...
void cs_main([[cppsl::builtin(dispatch_thread_id)]] uint3 dispatch_thread_id)
{
    ...
    float2 texcoords = float2{1.0f / float(cb.tex_width), 1.0f / float(cb.tex_height)} * (xy(dispatch_thread_id) + 0.5f);
    float3 normal = get_dir_from_latlong(texcoords);

    uint num_samples = uint(8192.0f * cb.roughness * cb.roughness);
    float pixels_per_omega = (float(cb.mip_0_width) * float(cb.mip_0_height)) / (4.0f * PI);
    ...
    for (uint i = 0u; i < num_samples; ++i)
    {
        ...
        if (n_dot_l > 0.0f)
        {
            ...
            float3 radiance = xyz(g_src_mip.SampleLevel(g_sampler, get_latlong_from_dir(l), mip_level));
    ...
    g_dst_mip.Store(pixel, make_float4(prefilter_color, 1.0f));
}

    ...
    namespace RHI
    {
        ...
        R<ITexture*> SwapChain::get_current_back_buffer()
        {
            @autoreleasepool
            {
                ...
                {
                    m_current_drawable = [m_metal_layer nextDrawable];
                    id<MTLTexture> texture = [m_current_drawable texture];
        ...
        RV SwapChain::present()
        {
            @autoreleasepool
            {
                lutry
                {
                    ...
                    id<MTLCommandQueue> queue = m_device->m_queues[m_command_queue_index].queue;
                    id<MTLCommandBuffer> buffer = [queue commandBuffer];
                    [buffer presentDrawable:m_current_drawable];
                    [buffer commit];
        ...
        RV SwapChain::init_metal_layer(const SwapChainDesc& desc)
        {
            @autoreleasepool
            {
                ...
                m_metal_layer = layer;
                m_metal_layer.device = m_device->m_device;

...
namespace Luna
{
    namespace GUI
    {
        namespace Internal
        {
            ...
            static f32 tab_width(GUICore::IContext* context, const GUICore::ElementHandle& element, const c8* label)
            {
            ...
            static RV layout_tab_bar(GUICore::IContext* context, const GUICore::ElementHandle& element,
                const RectF& rect, void* userdata)
            {
                TabAction* action = (TabAction*)userdata;
                if(!action || !action->state) return BasicError::bad_arguments();
                const GUICore::Element* bar = context->get_element(element.index);
                ...
                GUICore::LayoutResult result = bar->layout_result;
                result.content_size = Float2U(rect.width, rect.height);
                context->set_layout_result(element, result);
                return ok;
            }

            static RV draw_tab_bar(GUICore::IContext* context, const GUICore::ElementHandle& element,
            ...
            static RV draw_tab_header(GUICore::IContext* context, const GUICore::ElementHandle& element,
                GUICore::DrawPhase, void* userdata)
            {
                ...
                if(data->enabled && interaction.hovered && !selected)
                {
                    ...
                    background.type = GUICore::DrawCommandType::rect;
        ...
        LUNA_GUI_API GUICore::ElementHandle begin_tab_bar(GUICore::IContext* context, id_t id, i32* selected_index,
            const GUICore::LayoutConfig& layout, const TabBarDesc& desc)
        {
            luassert(context && id && selected_index);
            GUICore::ElementHandle bar = Internal::begin_element(context, id, "Tab Bar", layout);
        ...
        LUNA_GUI_API bool begin_tab_item(GUICore::IContext* context, id_t id, const c8* label)
        {
            ...
            header_layout.width.kind = GUICore::SizeKind::fit;
            header_layout.height.kind = GUICore::SizeKind::fixed;
            header_layout.height.value = Internal::style_scalar(context, scope.bar, "gui.tab.height", 32.0f);
            GUICore::ElementHandle header = Internal::begin_element(context, id, label ? label : "Tab", header_layout);
            ...
            if(selected)
            {
                ...
                scope.content = Internal::begin_element(context, GUICore::make_scoped_id(id, "content"),
        ...
        LUNA_GUI_API void end_tab_item(GUICore::IContext* context)
        {
            ...
            Internal::set_flex_layout(context, scope.content, flex, GUICore::LayoutAxis::y);

...
namespace Luna
{
    namespace GUI
    {
        namespace Internal
        {
            ...
            static f32 resolve_icon_size(GUICore::IContext* context,
                const GUICore::ElementHandle& element, const IconDesc& desc)
            {
                if(desc.size > 0.0f) return desc.size;
                return style_scalar(context, element, "gui.icon.size", 16.0f);
            }

            static GUICore::MeasureResult measure_icon(GUICore::IContext* context,
                const GUICore::ElementHandle& element, const Float2U&, void* userdata)
            {
            ...
            static RV draw_icon(GUICore::IContext* context, const GUICore::ElementHandle& element,
        ...
        LUNA_GUI_API GUICore::ElementHandle icon(GUICore::IContext* context, id_t id, IconName value,
            const GUICore::LayoutConfig& layout, const IconDesc& desc)
        {
            luassert(context && id);
            GUICore::ElementHandle element = Internal::begin_element(context, id, "Icon", layout);

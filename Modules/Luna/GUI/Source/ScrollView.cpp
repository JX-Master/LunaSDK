...
namespace Luna
{
    namespace GUI
    {
        namespace Internal
        {
            ...
            static RV layout_scroll_view(GUICore::IContext* context, const GUICore::ElementHandle& element,
                const RectF& rect, void* userdata)
            {
                ...
                lutry(GUICore::layout_scroll_viewport(context, element, rect, action->layout_desc));
                const GUICore::Element* viewport = context->get_element(element.index);
                if(!viewport) return BasicError::bad_arguments();
                u32 content_index = viewport->first_child;
                Float2U content_size(0.0f);
                if(content_index != GUICore::INVALID_ELEMENT)
                {
                    const GUICore::Element* content = context->get_element(content_index);
                    if(content)
                    {
                ...
                for(u32 child_index = content_index == GUICore::INVALID_ELEMENT ? GUICore::INVALID_ELEMENT :
                    context->get_element(content_index)->next_sibling;
                    child_index != GUICore::INVALID_ELEMENT;)
                {
                    ...
                    context->set_layout_result(GUICore::ElementHandle { child->id, child_index, element.generation }, result);
                    child_index = child->next_sibling;
                }
                GUICore::LayoutResult parent_result = viewport->layout_result;
                parent_result.content_size = content_size;
                context->set_layout_result(element, parent_result);
                return ok;
            }

            static RV draw_scrollbar(GUICore::IContext* context, const GUICore::ElementHandle& element,
                GUICore::DrawPhase, void* userdata)
            {
                ScrollbarData* data = (ScrollbarData*)userdata;
                if(!data || !data->state) return ok;
                const GUICore::Element* viewport = context->find_element(data->viewport_id);
                const GUICore::Element* bar = context->find_element(element.id);
            ...
            bool resolve_scroll_action(GUICore::IContext* context, ScrollAction& action)
            {
                ...
                const GUICore::Element* viewport = context->find_element(action.id);
                ...
                {
                    const GUICore::Element* bar = context->find_element(bar_id);
                    if(!bar) return;
                    for(const GUICore::RoutedInputEvent& routed : context->get_routed_input_events(bar_id))
                    {
        ...
        LUNA_GUI_API GUICore::ElementHandle begin_scroll_view(GUICore::IContext* context, id_t id, const c8* label,
            const GUICore::LayoutConfig& layout, const ScrollViewDesc& desc)
        {
            luassert(context && id);
            GUICore::LayoutConfig viewport_layout = layout;
            ...
            GUICore::ElementHandle viewport = Internal::begin_element(context, id, label ? label : "Scroll View",
                viewport_layout);
            ...
            action->id = id;
            action->horizontal_bar_id = GUICore::make_scoped_id(id, "horizontal_scrollbar");
            action->vertical_bar_id = GUICore::make_scoped_id(id, "vertical_scrollbar");
            action->desc = desc;
            action->state = state.get();
            action->layout_desc = Internal::allocate_frame<GUICore::ScrollViewportLayoutDesc>(context);
            action->layout_desc->scroll_offset = state->offset;
            ...
            content_layout.width.kind = GUICore::SizeKind::fit;
            content_layout.height.kind = GUICore::SizeKind::fit;
            GUICore::ElementHandle content = Internal::begin_element(context, GUICore::make_scoped_id(id, "content"),
        ...
        LUNA_GUI_API void end_scroll_view(GUICore::IContext* context)
        {
            ...
            Internal::set_flex_layout(context, scope.content, content_flex, GUICore::LayoutAxis::y);
            auto add_bar = [&](id_t bar_id, bool vertical, bool enabled)
            {
                ...
                GUICore::ElementHandle bar = Internal::begin_element(context, bar_id,
        ...
        {
            return GUICore::get_scroll_viewport_visible_rect(context, scroll_view);
        }
    }
}

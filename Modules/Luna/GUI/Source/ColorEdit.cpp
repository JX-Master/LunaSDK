...
namespace Luna
{
    namespace GUI
    {
        namespace Internal
        {
            ...
            static void draw_rect(GUICore::IContext* context, GUICore::DrawCommandType type, const RectF& rect,
            ...
            static void draw_gradient(GUICore::IContext* context, const RectF& rect, const Float4U& top_left,
            ...
            static void draw_outline(GUICore::IContext* context, const RectF& rect, const Float4U& color)
            {
            ...
            static void draw_label(GUICore::IContext* context, const RectF& rect, const c8* text,
                const Float4U& color, f32 font_size, VG::TextAlignment alignment = VG::TextAlignment::begin)
            {
                ...
                command.font = style_name(context, GUICore::ElementHandle(), "gui.font");
            ...
            static void draw_swatch(GUICore::IContext* context, const RectF& rect, const Float4U& color,
                const Float4U& border)
            {
                draw_rect(context, GUICore::DrawCommandType::rounded_rect, rect, border, 3.0f);
                ...
                for(u32 y = 0; y < rows; ++y)
                {
                    for(u32 x = 0; x < columns; ++x)
                    {
                        ...
                        draw_rect(context, GUICore::DrawCommandType::rect, cell,
                            (x + y) & 1 ? Float4U(0.20f, 0.23f, 0.28f, 1.0f) :
                            Float4U(0.40f, 0.44f, 0.50f, 1.0f));
                    }
                }
                draw_rect(context, GUICore::DrawCommandType::rounded_rect, inner, color, 2.0f);
            }
            ...
            static RV draw_color_preview(GUICore::IContext* context, const GUICore::ElementHandle& element,
                GUICore::DrawPhase, void* userdata)
            {
                ...
                draw_rect(context, GUICore::DrawCommandType::rounded_rect, RectF(), border, radius);
                draw_rect(context, GUICore::DrawCommandType::rounded_rect, RectF(1.0f, 1.0f, -2.0f, -2.0f), background,
            ...
            static RV draw_color_picker(GUICore::IContext* context, const GUICore::ElementHandle& element,
                GUICore::DrawPhase, void* userdata)
            {
                ...
                draw_rect(context, GUICore::DrawCommandType::rounded_rect, RectF(cursor_x - 8.0f, cursor_y - 8.0f, 16.0f, 16.0f),
                    cursor, 8.0f);
                draw_rect(context, GUICore::DrawCommandType::rounded_rect, RectF(cursor_x - 5.0f, cursor_y - 5.0f, 10.0f, 10.0f),
                    color, 5.0f);
                f32 bar_y = geometry.bar.offset_y + (axis == 0 ? bar : (1.0f - bar)) * geometry.bar.height;
                GUICore::DrawCommand marker;
                marker.type = GUICore::DrawCommandType::line;
                marker.rect_reference = GUICore::DrawCommandRectReference::element;
            ...
            static void build_channel_row(GUICore::IContext* context, id_t id, const c8* const* labels,
                i32* values, u32 count)
            {
                GUICore::LayoutConfig row_layout;
                row_layout.width.kind = GUICore::SizeKind::fixed;
                row_layout.width.value = 456.0f;
                row_layout.height.kind = GUICore::SizeKind::fixed;
                row_layout.height.value = 30.0f;
                GUICore::ElementHandle row = begin_h_layout(context, id, "Color Channels", row_layout);
                for(u32 i = 0; i < count; ++i)
                {
                    GUICore::LayoutConfig pair_layout;
                    pair_layout.width.kind = GUICore::SizeKind::fixed;
                    pair_layout.width.value = count == 1 ? 456.0f : 146.0f;
                    pair_layout.height.kind = GUICore::SizeKind::fixed;
                    pair_layout.height.value = 30.0f;
                    GUICore::ElementHandle pair = begin_h_layout(context, GUICore::make_scoped_id(id, (u64)i + 1),
                        "Color Channel", pair_layout);
                    GUICore::LayoutConfig label_layout;
                    label_layout.width.kind = GUICore::SizeKind::fixed;
                    label_layout.width.value = 20.0f;
                    label_layout.height.kind = GUICore::SizeKind::fixed;
                    label_layout.height.value = 30.0f;
                    text(context, GUICore::make_scoped_id(id, (u64)i + 11), labels[i], label_layout);
                    GUICore::LayoutConfig drag_layout;
                    drag_layout.width.kind = GUICore::SizeKind::fixed;
                    drag_layout.width.value = count == 1 ? 426.0f : 116.0f;
                    drag_layout.height.kind = GUICore::SizeKind::fixed;
                    drag_layout.height.value = 30.0f;
                    DragDesc drag_desc;
                    drag_desc.speed = 1.0f;
                    drag_int(context, GUICore::make_scoped_id(id, (u64)i + 31), &values[i], 0, 255, drag_layout, drag_desc);
            ...
            static void add_canvas_item(GUICore::CanvasLayoutItem& item, id_t id, f32 x, f32 y)
            {
                item.element_id = id;
            ...
            static void build_color_popup(GUICore::IContext* context, id_t id, const ColorBinding& binding,
                ColorEditState* state, const ColorEditDesc& desc)
            {
                ...
                canvas_layout.width.kind = GUICore::SizeKind::fixed;
                canvas_layout.width.value = 456.0f;
                canvas_layout.height.kind = GUICore::SizeKind::fixed;
                canvas_layout.height.value = canvas_height;
                GUICore::ElementHandle canvas = begin_canvas_layout(context, derived_id(id, "color_edit.canvas"),
                    "Color Edit Picker", canvas_layout);
                id_t picker_id = derived_id(id, "color_edit.picker");
                GUICore::LayoutConfig picker_layout;
                picker_layout.width.kind = GUICore::SizeKind::fixed;
                picker_layout.width.value = 456.0f;
                picker_layout.height.kind = GUICore::SizeKind::fixed;
                picker_layout.height.value = 300.0f;
                GUICore::ElementHandle picker = begin_element(context, picker_id, "Color Picker", picker_layout);
                ...
                GUICore::DrawConfig picker_draw;
                ...
                GUICore::LayoutConfig axis_layout;
                axis_layout.width.kind = GUICore::SizeKind::fixed;
                axis_layout.width.value = 456.0f;
                axis_layout.height.kind = GUICore::SizeKind::fixed;
                ...
                GUICore::CanvasLayoutItem items[5];
                ...
                canvas_desc.items = Span<const GUICore::CanvasLayoutItem>(items, alpha_id ? 5 : 4);
            ...
            bool resolve_color_edit_action(GUICore::IContext* context, ColorEditAction& action)
            {
                ...
                ensure_color_state(state);
                bool changed = false;
                GUICore::InteractionState preview = context->get_interaction_state(action.id);
                ...
                {
                    PickerGeometry geometry;
                    for(const GUICore::RoutedInputEvent& routed : context->get_routed_input_events(action.picker_id))
                    {
                        ...
                        else if(routed.event.type == GUICore::InputEventType::pointer_move && state.active_part)
                        {
                            Float4U before = read_color(binding);
                            apply_picker_position(binding, state, state.active_part, routed.element_position);
                            Float4U after = read_color(binding);
            ...
            static GUICore::ElementHandle color_edit(GUICore::IContext* context, id_t id, const c8* label,
                const ColorBinding& binding, const GUICore::LayoutConfig& layout, const ColorEditDesc& desc)
            {
                luassert(context && id);
                id_t popup_id = derived_id(id, "color_edit.popup");
                ...
                GUICore::ElementHandle preview = begin_element(context, id, label ? label : "Color Edit", layout);
                ...
                GUICore::LayoutCallbackConfig preview_layout;
                ...
                GUICore::DrawConfig preview_draw;
        ...
        LUNA_GUI_API GUICore::ElementHandle color_edit3(GUICore::IContext* context, id_t id, const c8* label,
            f32* value, const GUICore::LayoutConfig& layout, const ColorEditDesc& desc)
        {
        ...
        LUNA_GUI_API GUICore::ElementHandle color_edit3(GUICore::IContext* context, id_t id, const c8* label,
            u8* value, const GUICore::LayoutConfig& layout, const ColorEditDesc& desc)
        {
            Internal::ColorBinding binding;
        ...
        LUNA_GUI_API GUICore::ElementHandle color_edit3(GUICore::IContext* context, id_t id, const c8* label,
            u32* value, const GUICore::LayoutConfig& layout, const ColorEditDesc& desc)
        {
            Internal::ColorBinding binding;

...
namespace Luna
{
    namespace EditorGUI
    {
        namespace Internal
        {
            ...
            static GUI::MeasureResult measure_choice(GUI::IContext* context,
                ...
                GUI::DrawPhase, void* userdata)
            {
                ...
                else
                {
                    ...
                    if(data->enabled)
                    {
                        ...
                        RoundedRectEffect& shadow = track_effects[num_track_effects++];
                        shadow.shadow = true;
                        shadow.color = style_color(context, element, "gui.shadow.inset",
                            Float4U(0.0f, 0.0f, 0.0f, 0.18f));
                        shadow.shadow_desc.offset = Float2U(2.0f, 2.0f);
                        shadow.shadow_desc.softness = softness * 0.5f;
                    ...
                    RoundedRectEffect knob_effects[2];
                    ...
                    if(data->enabled)
                    {
                        ...
                        RoundedRectEffect& shadow = knob_effects[num_knob_effects++];
                        shadow.shadow = true;
                        shadow.color = style_color(context, element, "gui.shadow.dark",
                            Float4U(0.0f, 0.0f, 0.0f, 0.20f));
                        shadow.shadow_desc.offset = shadow_offset;
                        shadow.shadow_desc.softness = softness;
                    }
            ...
            static GUI::ElementHandle choice(GUI::IContext* context, id_t id, const c8* label,
                bool selected, ChoiceKind kind, const GUI::LayoutConfig& layout, const ChoiceDesc& desc,
                ChoiceOperation operation, bool* bool_value, i32* int_value, i32 set_value)
            {
                GUI::ElementHandle element = begin_element(context, id, label ? label : "Choice", layout);
        ...
        LUNA_EDITOR_GUI_API GUI::ElementHandle selectable(GUI::IContext* context, id_t id, const c8* label,
            bool selected, const GUI::LayoutConfig& layout, const ChoiceDesc& desc)
        {
        ...
        LUNA_EDITOR_GUI_API GUI::ElementHandle checkbox(GUI::IContext* context, id_t id, const c8* label,
            bool* value, const GUI::LayoutConfig& layout, const ChoiceDesc& desc)
        {
        ...
        LUNA_EDITOR_GUI_API GUI::ElementHandle radio_button(GUI::IContext* context, id_t id, const c8* label,
            i32* value, i32 button_value, const GUI::LayoutConfig& layout, const ChoiceDesc& desc)
        {

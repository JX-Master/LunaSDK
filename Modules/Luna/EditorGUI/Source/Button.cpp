...
namespace Luna
{
    namespace EditorGUI
    {
        namespace Internal
        {
                ...
                GUI::DrawPhase, void* userdata)
            {
                ...
                if(data->enabled && active > 0.001f)
                {
                    ...
                    inset.shadow_desc.mode = GUI::ShadowMode::inner;
                }
            ...
            {
                GUI::FlexLayoutDesc* flex = allocate_frame<GUI::FlexLayoutDesc>(context);
                flex->axis = GUI::LayoutAxis::x;
                flex->main_alignment = GUI::FlexAlignment::center;
        ...
        LUNA_EDITOR_GUI_API GUI::ElementHandle begin_button(GUI::IContext* context, id_t id, const c8* label,
            const GUI::LayoutConfig& layout, const ButtonDesc& desc)
        {
            luassert(context && id);
            ...
            Float4U padding_value = context->get_style_value(style, Name("gui.button.padding"),
                GUI::style_f32x2(Float2U(10.0f, 6.0f))).number;
            ...
            GUI::ElementHandle element = Internal::begin_element(context, id, label ? label : "Button",
                resolved_layout);
        ...
        LUNA_EDITOR_GUI_API GUI::ElementHandle text_button(GUI::IContext* context, id_t id, const c8* value,
            const GUI::LayoutConfig& layout, const ButtonDesc& desc)
        {
            GUI::ElementHandle element = begin_button(context, id, value, layout, desc);
            TextDesc text_desc;
            ...
            text(context, GUI::make_scoped_id(id, "text"), value, text_layout, text_desc);
            end_button(context);
            return element;
        }

        LUNA_EDITOR_GUI_API GUI::ElementHandle shape_button(GUI::IContext* context, id_t id, const c8* label,
            const GUI::ShapeDesc& value, const GUI::LayoutConfig& layout, const ShapeButtonDesc& desc)
        {
            ButtonDesc button_desc;
            button_desc.enabled = desc.enabled;
            GUI::ElementHandle element = begin_button(context, id, label, layout, button_desc);
            GUI::LayoutConfig shape_layout;
            shape_layout.width.kind = GUI::SizeKind::percent;
            shape_layout.width.value = 1.0f;
            shape_layout.height.kind = GUI::SizeKind::percent;
            ...
            shape(context, GUI::make_scoped_id(id, "shape"), value, shape_layout, shape_desc);
            end_button(context);
            return element;
        }
    }

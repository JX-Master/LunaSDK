...
namespace Luna
{
    namespace EditorGUI
    {
        namespace Internal
        {
            ...
            static Ref<DockSpaceState> dock_space_state(GUI::IContext* context, id_t id)
            {
                id_t state_id = GUI::make_state_id<DockSpaceState>(id);
                ...
                lupanic_if_failed(context->set_state(state_id, state.object(), GUI::StateLifetime::context));
                return state;
            }
            ...
            static f32 dock_tab_label_width(GUI::IContext* context,
                const GUI::ElementHandle& element, const c8* label, f32 font_size)
            {
            ...
            static f32 dock_tab_width(GUI::IContext* context,
                const GUI::ElementHandle& element, const c8* label, f32 font_size)
            {
            ...
            static f32 dock_tab_width_scale(GUI::IContext* context,
                const GUI::ElementHandle& element, DockSpaceAction& action,
            ...
            static RV layout_dock_space(GUI::IContext* context, const GUI::ElementHandle& element,
            ...
            static bool has_pointer_event(GUI::IContext* context, id_t id, GUI::InputEventType type,
                GUI::InputEvent* out_event = nullptr)
            {
                for(const GUI::InputEvent& event : context->get_delivered_input_events(id))
                {
                    if(event.type == type && (type != GUI::InputEventType::pointer_down &&
                        type != GUI::InputEventType::pointer_up || event.button == GUI::PointerButton::left))
                    {
            ...
            static void raise_panel(GUI::IContext* context, DockSpaceState& state, DockPanelActionInfo& panel)
            {
                DockPanelPersistentData* persistent = find_panel(state, panel.id);
            ...
            bool resolve_dock_space_action(GUI::IContext* context, DockSpaceAction& action)
            {
                ...
                for(usize i = 0; i < action.panel_count; ++i)
                {
                    ...
                    if(panel.raise.id && has_pointer_event(context, panel.raise.id, GUI::InputEventType::pointer_down))
                    {
                ...
                GUI::InputEvent root_event;
                if(has_pointer_event(context, action.id, GUI::InputEventType::pointer_down, &root_event))
                {
                ...
                for(const GUI::InputEvent& event : context->get_input_events())
                {
                    if(event.type == GUI::InputEventType::pointer_up && event.button == GUI::PointerButton::left)
                    {
        ...
        LUNA_EDITOR_GUI_API GUI::ElementHandle begin_dock_space(GUI::IContext* context, id_t id,
            const c8* label, const GUI::LayoutConfig& layout, const DockSpaceDesc& desc)
        {
            luassert(context && id);
            ...
            bool* open, const DockPanelDesc& desc)
        {
            ...
            {
                info.desc.title_bar_height = Internal::style_scalar(context, GUI::ElementHandle(),
            ...
            if(info.floating)
            {
                info.layer_id = GUI::make_scoped_id(Internal::derived_id(scope.id, "floating.panel"), id);
            ...
            {
                GUI::LayoutConfig title_layout;
                title_layout.width.kind = GUI::SizeKind::percent;
                title_layout.width.value = 1.0f;
                title_layout.height.kind = GUI::SizeKind::fixed;
        ...
        LUNA_EDITOR_GUI_API void end_dock_panel(GUI::IContext* context)
        {
            ...
            {
                GUI::LayoutConfig close_layout;
                close_layout.width.kind = GUI::SizeKind::fixed;
                close_layout.width.value = panel.desc.title_bar_height;
                close_layout.height.kind = GUI::SizeKind::fixed;
            ...
            {
                GUI::LayoutConfig resize_layout;
                resize_layout.width.kind = GUI::SizeKind::fixed;
                resize_layout.width.value = panel.desc.resize_border_size * 2.0f;
                resize_layout.height.kind = GUI::SizeKind::fixed;
            ...
            if(panel.floating)
            {
                GUI::LayoutConfig raise_layout;
                raise_layout.width.kind = GUI::SizeKind::percent;
                raise_layout.width.value = 1.0f;
                raise_layout.height.kind = GUI::SizeKind::percent;
            ...
            GUI::CanvasLayoutItem* items = Internal::allocate_frame_array<GUI::CanvasLayoutItem>(context, 5);
            usize item_count = 0;
            auto add_item = [&](const GUI::ElementHandle& element, const Float2U& anchor_min,
                const Float2U& anchor_max, const Float2U& pivot, const Float4U& offset)
            {
                if(!element.id) return;
                GUI::CanvasLayoutItem& item = items[item_count++];
            ...
            GUI::CanvasLayoutDesc* canvas = Internal::allocate_frame<GUI::CanvasLayoutDesc>(context);
            canvas->items = Span<const GUI::CanvasLayoutItem>(items, item_count);

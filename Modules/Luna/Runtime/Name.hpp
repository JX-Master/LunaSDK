...
namespace Luna
{
	...
	//! @brief Interns one name string to the runtime and fetches the interned address for it.
	...
	//! @brief Increases the reference count of the name string by 1.
	...
	//! @brief Decreases the reference count of the name string by 1, and eventually frees the name string when the reference count goes to 0.
	//! @param[in] name The pointer of the string. If this is `nullptr`, this call does nothing.
	//! @par Valid Usage
	//! * If `name` is not `nullptr`, it must be a string pointer returned by @ref intern_name.
	LUNA_RUNTIME_API void release_name(const c8* name);

	//! @brief Gets the ID for the specified name. The ID keeps constant between multiple processes.
	...
	//! @brief Fetches the size of the name string.
	...
	//! @brief Represents one name string.
	...
	private:
		const c8* m_str;
	public:
		//! @brief Constructs one empty name.
		Name() :
			m_str(nullptr) {}
		//! @brief Constructs one name with the provided name string.
		//! @param[in] name The name string.
		//! @par Valid Usage
		//! * If `name` is not `nullptr`, it must be null-terminated.
		Name(const c8* name) :
			m_str(intern_name(name)) {}
		//! @brief Constructs one name with the provided name string and size.
		//! @param[in] name The name string.
		...
		//! @brief Constructs one name from one string.
		//! @param[in] str The name string.
		Name(const String& str) :
			m_str(intern_name(str.c_str())) {}
		//! @brief Constructs one name from one substring of the provided string.
		//! @param[in] str The name string.
		...
		//! @brief Gets the internal string pointer of this name.
		...
		//! @brief Gets the size of the name string.
		//! @return Returns the size of the name string.
		usize size() const
		{
			return get_name_size(m_str);
		}
		//! @brief Gets the ID of the name string.
		...
		//! @brief Clears the name string and resets it to one empty name.
		void reset()
		{
			release_name(m_str);
			m_str = nullptr;
	...
	//! @brief Gets the type object of @ref Name.

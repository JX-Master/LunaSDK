...
namespace Luna
{
	//! @brief Reports an assertion failure information to the underlying OS or CRT.
	...
	//! @brief Triggers a debug break, pauses the program and attaches the debugger to the program.
...
//! @brief Evaluates the given expression, and if the result value cannot pass `if` statement, calls `assert_fail`.
...
//! @brief Same as @ref luassert_always, but displays a custom message instead of the expression.
...
//! @brief Triggers an assertion failure directly.
#define lupanic_always() luassert_msg_always(false, "Panic has been called.")

//! @brief Triggers an assertion failure with custom message.
#define lupanic_msg_always(_message) luassert_msg_always(false, _message)

#ifdef LUNA_DEBUG
//! @brief Checks whether the specified condition evaluates to a non-zero value, and reports an assertion failure if not.
...
//! @brief Reports an assertion failure immediately.
//! @details This only works in Debug compile mode.
#define lupanic() lupanic_always()
//! @brief Checks whether the specified condition evaluates to a non-zero value, and reports an assertion failure with the user-defined message if not.
//! @details This only works in Debug compile mode.
//! @param[in] _condition The condition to evaluate.

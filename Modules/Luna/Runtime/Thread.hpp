...
namespace Luna
{
	...
	//! @brief Specifies the thread schedule priority.
	enum class ThreadPriority : i32
	{
		//! @brief The low priority.
		low = 0,
		//! @brief The default priority assigned by the system when the thread is created.
		normal = 1,
		//! @brief The high priority.
		high = 2,
		//! @brief The highest priority possible for the system.
		critical = 3
	};
	
	//! @interface IThread
	//! @threadsafe
	//! @brief Represents a system thread object.
	...
	//! @brief Gets the number of logical processors on the platform.
	...
	//! @brief Create a new system thread and make it run the callback function. The thread will be closed when the callback function returns.
	...
	//! @brief Delays the execution of this thread for a very shout time by yielding this thread several times.
	...
	//! @brief Yields the remain time slice of the current thread and let OS to schedule other threads.
	...
	//! @brief Allocates one thread local storage (TLS) slot.
	...
	//! @brief Frees the TLS slot allocated by `tls_alloc`. 
	...
	//! @brief Set the data bound to the current thread's TLS slot specified by `handle`.
	...
	//! @brief Get the value bound to the TLS slot of current thread.

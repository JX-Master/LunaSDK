...
namespace Luna
{
	...
	//! @brief Represents file binary attributes.
	...
	//! @brief Specifies attributes for one file open operation.
	...
	//! @brief Specifies file attributes.
	...
	//! @interface IFile
	//! @brief Represents a opened file object.
	struct IFile : virtual ISeekableStream
	{
		luiid("{c61fbf68-89dc-4776-8a99-cc855ff8289e}");

		//! @brief Clears all buffers for this stream and causes all buffered data to be written to the underlying device.
		virtual void flush() = 0;
	};

	//! @interface IFileIterator
	//! @brief Represents a directory stream that can be used to iterate all files and directories
	...
	struct IFileIterator : virtual Interface
	{
		...
		//! @brief Checks if this iterator points to a valid file in the directory stream.
		//! @return Returns `true` if the iterator points to a valid file, returns `false` otherwise.
		virtual bool is_valid() = 0;
		
		//! @brief Gets the filename of the file that the iterator currently points to.
		...
		//! @brief Gets the file attribute flags of the file.
		...
		//! @brief Moves the iterator to the next file in the directory.
	...
	//! @brief Returns the data in the specified file as a blob object.
	...
	//! @brief Specify attributes for one file copy operation.
	...
	//! @brief Copies the file or directory from the source path to the destination path.
	...
	//! @brief Specify attributes for one file move operation.
	...
	//! @brief Moves the file or directory from the source path to the destination path. This call can also be used to rename a file.
	//! @param[in] from_path Source file or directory path.
	//! @param[in] to_path Destination file or directory path.
	...
	//! @brief Creates a file iterator that can be used to iterate all files in the specified directory.
	...
	//! @brief Creates one empty directory.
	...
	//! @brief Gets the current working directory path for the underlying system.
	...
	//! @brief Sets the current working directory path for the underlying system. The current directory will be set for the process scope.
	//! @param[in] path The current working directory path to set.
	LUNA_RUNTIME_API RV set_current_dir(const c8* path);
	//! @brief Gets the full (absolute) path of the application's executable file.

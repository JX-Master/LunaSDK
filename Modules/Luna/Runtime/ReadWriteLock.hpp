...
namespace Luna
{
	...
	//! @brief Represents one system-level read write lock.
	...
	struct IReadWriteLock : virtual Interface
	{
		...
		//! @brief Acquires one read ownership of the lock.
		//! @details This operation blocks the current thread until the ownership is acquired.
		virtual void acquire_read() = 0;
		//! @brief Acquires one write ownership of the lock.
		//! @details This operation blocks the current thread until the ownership is acquired.
		virtual void acquire_write() = 0;
		//! @brief Tries to acquire one read ownership of the lock.
		...
		//! @brief Tries to acquire one write ownership of the lock.
		//! @details This operation does not block the current thread, it returns immediately no matter whether the ownership is 
		//! acquired.
		//! @return Returns `true` if the ownership is acquired when this function returns. Returns `false` otherwise.
		virtual bool try_acquire_write() = 0;
		//! @brief Releases the read ownership acquired by @ref acquire_read or @ref try_acquire_read.
		virtual void release_read() = 0;
		//! @brief Releases the write ownership acquired by @ref acquire_write or @ref try_acquire_write.
		virtual void release_write() = 0;
	};

	//! @brief Creates one new read write lock.

...
namespace Luna
{
	...
	public:
		...
		//! @remark Since this set does not allow inserting multiple elements with the same key, the returned range 
		...
		//! set if such element is not found. The key is extracted from the value.
		//! @param[in] value The element value to assign or insert.
		//! @return Returns one iterator-bool pair indicating the result:
		//! * If the returned Boolean value is `true`, then the element is inserted to the set, and the 
		//! returned iterator points to the inserted element.
		//! * If the returned Boolean value is `false`, then one existing element is found and is assigned to the 
		//! specified value, and the returned iterator points to the existing element in the set.

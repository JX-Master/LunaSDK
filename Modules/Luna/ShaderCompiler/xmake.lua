-- spirv-cross will fail on Windows using clang-cl
if is_os("windows") then 
    add_requireconfs("spirv-cross", {configs = {toolchains = "msvc", exceptions = false}})
end

if is_os("windows", "macosx") then
    add_requires("spirv-cross")
end

luna_sdk_module_target("ShaderCompiler")
    add_headerfiles("*.hpp", {prefixdir = "Luna/ShaderCompiler"})
    add_headerfiles("Source/**.hpp", {install = false})
    add_files("Source/*.cpp")
    add_deps("Runtime", "VariantUtils")
    if is_os("windows", "macosx") then
        add_packages("spirv-cross")
        if is_os("windows") then 
            add_includedirs("$(projectdir)/SDKs/vulkan-tools/windows/include")
            if is_arch("x64") then 
                add_linkdirs("$(projectdir)/SDKs/vulkan-tools/windows/x64/lib")
                add_installfiles("$(projectdir)/SDKs/vulkan-tools/windows/x64/bin/dxcompiler.dll", {prefixdir = "bin", public = true})
            elseif is_arch("arm64") then 
                add_linkdirs("$(projectdir)/SDKs/vulkan-tools/windows/arm64/lib")
                add_installfiles("$(projectdir)/SDKs/vulkan-tools/windows/arm64/bin/dxcompiler.dll", {prefixdir = "bin", public = true})
            end
        elseif is_os("macosx") then
            add_includedirs("$(projectdir)/SDKs/vulkan-tools/macosx/arm64/include")
            add_cxflags("-fms-extensions")
            add_linkdirs("$(projectdir)/SDKs/vulkan-tools/macosx/arm64/lib")
            add_installfiles("$(projectdir)/SDKs/vulkan-tools/macosx/arm64/lib/libdxcompiler.dylib", {prefixdir = "bin", public = true})
            add_rpathdirs("@executable_path/.")
        end
        add_links("dxcompiler")
        after_build(function(target)
            if target:is_plat("windows") then 
                if target:is_arch("x64") then 
                    os.cp("$(projectdir)/SDKs/vulkan-tools/windows/x64/bin/dxcompiler.dll", path.join(target:targetdir(), "dxcompiler.dll"))
                elseif target:is_arch("arm64") then 
                    os.cp("$(projectdir)/SDKs/vulkan-tools/windows/arm64/bin/dxcompiler.dll", path.join(target:targetdir(), "dxcompiler.dll"))
                end
            elseif target:is_plat("macosx") then 
                os.cp("$(projectdir)/SDKs/vulkan-tools/macosx/arm64/lib/libdxcompiler.dylib", path.join(target:targetdir(), "libdxcompiler.dylib"))
            end
        end)
        after_clean(function(target)
            if target:is_plat("windows") then 
                os.rm(path.join(target:targetdir(), "dxcompiler.dll"))
            elseif target:is_plat("macosx") then 
                os.rm(path.join(target:targetdir(), "libdxcompiler.dylib"))
            end
        end)
    end
target_end()

...
* 
* @file MipmapGeneration3DCS.hlsl
* @author JXMaster
* @date 2025/9/6
*/
...
{
    float3 texel_size;    // 1.0 / destination dimension
}
Texture3D<float4> g_src_tex : register(t1);
RWTexture3D<float4> g_dst_tex : register(u2);
...
void main(uint3 dispatch_thread_id : SV_DispatchThreadID)
{
    ...
    float3 texcoords = texel_size * (dispatch_thread_id.xyz + 0.5f);
    ...
    g_dst_tex[dispatch_thread_id.xyz] = color;
}
...
namespace Luna
{
	...
	//! This container is similar to `SelfIndexedUnorderedMap`, but allows multiple elements with the same key to 
	//! exist.
	template <
		typename _Kty,
		typename _Ty,
	...
	template <
		...
		typename _KeyEqual,
		typename _Alloc>
	inline void gc_track(const SelfIndexedUnorderedMultiMap<_Kty, _Ty, _ExtractKey, _Hash, _KeyEqual, _Alloc>& obj)
	{
		for (auto& i : obj)
		{
			gc_track(i);

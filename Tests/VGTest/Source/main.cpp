...
int luna_main(int argc, const char* argv[])
{
    ...
    lutry
    {
        ...
        Window::set_event_handler([](object_t event, void* userdata)
        {
            ...
            {
                if(window_event->window == app->window)
                {
                    if(auto e = cast_object<Window::WindowMouseDownEvent>(event))
                    {
                        if (e->button == HID::MouseButton::right)
                        {
                    ...
                    else if(auto e = cast_object<Window::WindowFramebufferResizeEvent>(event))
                    {

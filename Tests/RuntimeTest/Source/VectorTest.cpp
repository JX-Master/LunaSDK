...
namespace Luna
{
	void vector_test()
	{
		TestObject::reset();
		{
			...
			const Vector<i32> vec4(10, 12);
			lutest(vec4.size() == 10);
			lutest(vec4[9] == 12);

			const Vector<TestObject> vec5(10);
			lutest(vec5.size() == 10);
			lutest(vec5[4] == TestObject(0));

			// Copy ctor.
			const Vector<TestObject> vec6(vec5);
			lutest(vec6.size() == 10);
			...
			Vector<int> vec8(A, A + N);
			lutest(equal(vec8.begin(), vec8.end(), A));
			lutest(vec8.size() == N);

			// Value and count ctor.
			Vector<int> vec9(10, 5);
			lutest(vec9.size() == 10);
			for (int i : vec9)
			{
				lutest(i == 5);
...

		{
			...
			// assign from integer count and value.
			vec2.assign(10, 5);
			for (int i : vec2)
			{
				lutest(i == 5);
...

		{
			...
			// iterator swap_erase(iterator position);
			// void     clear();
			...
			vec.swap_erase(vec.begin()); // Becomes: 6, 5.
			lutest(vec.size() == 2);
			lutest(vec[0] == 6);
			lutest(vec[1] == 5);

			vec.swap_erase(vec.begin() + 1); // Becomes: 6.
			lutest(vec.size() == 1);
			lutest(vec[0] == 6);

...
namespace Luna::GUICoreTest
{
    namespace
    {
        ...
        RV draw_sdf_sample(GUICore::IContext* context, const GUICore::ElementHandle& element,
        ...
        void build_sdf_sample(GUICore::IContext* context, SDFSample sample)
        {
            GUICore::ElementHandle element = context->begin_element(sample_id(sample));
            context->set_layout_config(element, fixed_layout(TILE_WIDTH, TILE_HEIGHT));
            GUICore::DrawConfig draw;
            draw.name = Name("guicore.test.sdf.sample");
    ...
    {
        GUICore::ElementHandle section = context->begin_element(ID_SDF);
        ...
        state.sdf_canvas.items = Span<const GUICore::CanvasLayoutItem>(state.sdf_items.data(), state.sdf_items.size());
        state.sdf_canvas.default_item = GUICore::CanvasLayoutItem();

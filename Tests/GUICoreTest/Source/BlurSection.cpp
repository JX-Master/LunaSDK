...
namespace Luna::GUICoreTest
{
    namespace
    {
        ...
        void draw_backdrop(GUICore::IContext* context, const RectF& rect, f32 radius)
        {
        ...
        RV draw_animated_background(GUICore::IContext* context,
            const GUICore::ElementHandle&, GUICore::DrawPhase, void* userdata)
        {
        ...
        RV draw_glass_panel(GUICore::IContext* context,
            const GUICore::ElementHandle&, GUICore::DrawPhase, void* userdata)
        {
        ...
        void build_glass_panel(GUICore::IContext* context, GUICore::id_t id,
            const GlassMaterialSample& material)
        {
            ...
            draw.name = Name("guicore.test.blur.material");
    ...
    {
        GUICore::ElementHandle section = context->begin_element(ID_BLUR);
        context->set_layout_config(section,
            fixed_layout(SECTION_WIDTH, SECTION_HEIGHT));

        GUICore::ElementHandle background =
            context->begin_element(ID_BLUR_BACKGROUND);
        context->set_layout_config(background,
            fixed_layout(SECTION_WIDTH, SECTION_HEIGHT));
        GUICore::DrawConfig background_draw;
        background_draw.name = Name("guicore.test.blur.background");
        ...
        state.blur_canvas.items = Span<const GUICore::CanvasLayoutItem>(
            state.blur_items.data(), state.blur_items.size());
        state.blur_canvas.default_item = GUICore::CanvasLayoutItem();

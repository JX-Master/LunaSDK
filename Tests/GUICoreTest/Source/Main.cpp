...
namespace
{
    ...
    RV draw_blur_planning_sample(GUICore::IContext* context,
        const GUICore::ElementHandle&, GUICore::DrawPhase, void*)
    {
        GUICore::DrawCommand command;
        command.type = GUICore::DrawCommandType::backdrop_blur;
    ...
    RV run_blur_planning_case(CoreTestApp& app, u32 size,
        f32 softness, u8 downsample_level,
        const GUICore::RendererPerformanceCounters& expected)
    {
        lutry
        {
            ...
            draw.name = Name("guicore.test.blur.planning");
            ...
            lulet(texture, RHI::get_main_device()->new_texture(
                RHI::MemoryType::local, RHI::TextureDesc::tex2d(
                    ...
                    size, size, 1, 1)));
            GUICore::RenderTargetDesc target(texture);
    ...
    RV validate_blur_filter_planning(CoreTestApp& app)
    {
        GUICore::RendererPerformanceCounters snapshot;
        ...
        GUICore::RendererPerformanceCounters quantized_subpixel;
        ...
        GUICore::RendererPerformanceCounters downsample_only;
        ...
        GUICore::RendererPerformanceCounters automatic_downsample;
    ...
    RV resize_gui_target(CoreTestApp& app, const UInt2U& size)
    {
        lutry
        {
    ...
    RV run_core_test(u32 initial_slice, u32 max_frames)
    {
        lutry
        {
            ...
            if(app.sheet.slice_index == GUICoreTest::BLUR_MATERIALS_SLICE)
            {
            ...
            while(true)
            {
                ...
                if(app.sheet.slice_index == GUICoreTest::BLUR_MATERIALS_SLICE)
                {
                    ...
                    u32 blur_commands = 0;
                    for(const GUICore::DrawCommand& command : generated_commands)
                    {
                ...
                if(app.sheet.slice_index == GUICoreTest::BLUR_MATERIALS_SLICE)
                {
                    const GUICore::RendererPerformanceCounters& counters =
                        app.renderer->get_performance_counters();
...
int luna_main(int argc, const char* argv[])
{
    ...
    for(int i = 1; i < argc; ++i)
    {
        ...
        {
            initial_slice = GUICoreTest::BLUR_MATERIALS_SLICE;
        }
